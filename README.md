...
## 🌐 在线示例
项目已部署示例站点，可通过以下地址查看线上运行效果：

> **示例站点：** <https://yolo.tyut.pp.ua>  
> **演示密码：** `admin123`

该站点用于展示系统的登录、初始化、实时监控、物品检索、统计分析和客户端推流等主要功能。由于系统涉及摄像头调用、历史快照和后台管理操作，访问受管理员登录保护；示例站点可使用演示密码 `admin123` 登录，自行部署时请使用部署时配置的 `ADMIN_PASSWORD`。
...
- **🌐 全场景推流支持 (双模混动)：**
  - **云端拉流模式：** 支持直接拉取远程 RTSP/RTMP/HTTP 网络监控流，服务端推理后通过 MJPEG 回显检测画面。
  - **客户端推流模式 (WebSocket Push)：** 支持在浏览器端直接调用本地笔记本摄像头或打开本地视频文件，通过 WebSocket 以二进制帧方式实时推送至服务端 GPU 进行 YOLO 推理，并将实时画框结果无缝回显。
...
### 3. 登录系统
配置完域名与 HTTPS 后，通过浏览器访问您的域名 `https://your-domain.com`，系统会自动跳转至**登录页面**。本项目示例部署地址为 <https://yolo.tyut.pp.ua>，演示密码为 `admin123`。自行部署时，请使用您在 `ADMIN_PASSWORD` 中设置的密码登录（默认为 `admin123`）。
